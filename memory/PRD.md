...
- **Wallet in INR**, keyed to the account (`user:<id>`), merged from the anonymous device wallet on first login. Prices: analysis ₹20, compare ₹32, portfolio ₹4. Re-check is free unless price moved >1.5% or crossed the verdict's target/stop (`wallet.should_charge_for_recheck`). `WALLET_ENFORCEMENT_ENABLED` is **unset (OFF)** — analyses are currently free.
