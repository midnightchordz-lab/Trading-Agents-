...
def test_a_chain_shorter_than_the_trusted_hops_falls_back_leniently():
    """Not through the expected edge (a local call, a test, or a topology
    change). Deliberately lenient: reading a proxy's own address here would
    collapse every real user into one bucket and lock them all out of sign-in.
    The global budgets bound this case instead."""
    assert ar.client_ip(FakeRequest("203.0.113.5")) == "203.0.113.5"
    assert ar.client_ip(FakeRequest("203.0.113.5,198.51.100.2")) == "203.0.113.5"
...
def test_the_overall_budget_stops_email_pumping_too(clean_otp_window):
    tag = clean_otp_window
    seed(ar.OTP_GLOBAL_MAX_PER_HOUR, "email", tag)
...
def test_an_hour_old_burst_does_not_count(clean_otp_window):
    ...
    one attack."""
    tag = clean_otp_window
    seed(ar.OTP_GLOBAL_MAX_PER_HOUR + 50, "email", tag, minutes_ago=61)
    res = request_otp(f"{uuid.uuid4().hex[:10]}@gmail.com")
    assert res.status_code != 429, res.text
...
def test_normal_traffic_is_nowhere_near_the_budget(clean_otp_window):
    ...
    assert ar.OTP_GLOBAL_MAX_PER_HOUR >= 100
    assert ar.OTP_GLOBAL_MAX_PER_HOUR > ar.OTP_SMS_GLOBAL_MAX_PER_HOUR
